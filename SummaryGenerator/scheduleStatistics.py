...
### TODO: delete as no longer using.
def plot_pie_chart(data, labels, title, filename):
    ...
    legend_labels = [
        f"{label}: {percent:.2f}%"
        for label, percent in zip(labels, 100 * data / sum(data))
    ]
...
def plot_scans_and_observations(args, station_data, stats_data):
    ...
    sessions_df = pd.DataFrame(
        station_data["sessions"]["rows"], columns=station_data["sessions"]["columns"]
    )
...
def get_glovdh_barchart(station, time_start, time_stop, is_vgos):
    ...
    if (
        ...
        for station in stations:
            ...
            df = pd.DataFrame(
                data["sessions"]["rows"], columns=data["sessions"]["columns"]
            )
    ...
    top_stations = stat_sessions_df.sort_values(
        by="total_sessions", ascending=False
    ).head(top_station_num)
