...
def determineAssignmentRate(table_list, stat_tab_list, target_stat):
    ...
    for exp in table_list[station_index]["ExpID"]:
        exp_date = table_list[station_index]["Date"][
            table_list[station_index]["ExpID"] == exp
        ][0]
        target_stat_obs = table_list[station_index]["Total_Obs"][
            table_list[station_index]["ExpID"] == exp
        ][0]
        ...
        for i in range(0, len(table_list)):
            if exp in table_list[i]["ExpID"]:
                station_obs = table_list[i]["Total_Obs"][table_list[i]["ExpID"] == exp][
                    0
                ]
...
def plotAssignmentRate(ass_rate):
    ...
    ax.fill_between(
        ass_rate_array[:, 1], ass_rate_array[:, 2] * 100, color="steelblue", alpha=0.5
    )
    ax.hlines(
        median_ass_rate * 100,
        np.min(ass_rate_array[:, 1]),
        np.max(ass_rate_array[:, 1]),
        linestyle="dashed",
        color="steelblue",
    )
    ...
    for i, txt in enumerate(ass_rate_array[:, 0]):
        ax.annotate(
            txt,
            (ass_rate_array[i, 1], ass_rate_array[i, 2] * 100),
            rotation=90,
            fontsize=5,
            xytext=(0, -20),
...
def medWRMSdelALL(table_list, stat_tab_list):
    ...
    for i in range(0, len(temp_table_list)):
        if len(temp_table_list[i]) > 0:
            med_wrms_list.append(
                [stat_tab_list[i], np.median(temp_table_list[i][col_name])]
            )
...
def plotBenchSess(data, specific_station):
    ...
    fig, ax = plt.subplots(figsize=(7, 4.5))
    bars = ax.bar(
        data[0:10, 0], data[0:10, 1].astype(float), color="steelblue", alpha=0.8
    )  # Plot the 10 best performing stations
    bar_specific = ax.bar(
        data[specific_stat_index, 0],
        data[specific_stat_index, 1].astype(float),
        color="firebrick",
        alpha=0.8,
    )  # Plot the 'target' station
