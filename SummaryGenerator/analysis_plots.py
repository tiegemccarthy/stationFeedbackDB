...
def wRmsAnalysis(table_input):
    ...
    ax.legend(
        [
            "Station W.RMS delay",
            "Session W.RMS delay",
            "Median Station W.RMS delay",
            "Median Session W.RMS delay",
        ],
        loc="upper left",
    )
    ax.set_xlabel("Date")
...
def performanceAnalysis(table_input):
    ...
    ax.hlines(
        np.median(table["Performance"]) * 100,
        np.min(time_data),
        np.max(time_data),
        linestyle="dashed",
        color="steelblue",
